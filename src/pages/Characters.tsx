...
export const Characters: React.FC = () => {
  ...
  const fetchCharacters = async () => {
    try {
      ...
      // Map static data to Character interface
      const staticData = [
        ...analyticsData.map((i, idx) => ({ id: 1000 + idx, image_url: i.image, person_name: i.subject, character_name: 'شخصية ثابتة' })),
        ...casesData.map((i, idx) => ({ id: 2000 + idx, image_url: i.image, person_name: i.subject, character_name: 'قضية ثابتة' })),
        ...wantedData.map((i, idx) => ({ id: 3000 + idx, image_url: i.image, person_name: i.name, character_name: 'مطلوب ثابت' })),
      ];
      setCharacters([...staticData, ...response.data]);
    } catch (error) {
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#0f172a] border border-white/10 p-8 rounded-3xl w-full max-w-md space-y-4 shadow-2xl">
              ...
              <button onClick={handleAdd} className="w-full bg-blue-600 p-3 rounded-xl text-white flex items-center justify-center gap-2">
                {isUploading ? <><Loader2 className="animate-spin" /> {uploadProgress}%</> : 'إضافة الصور/ة'}
              </button>
